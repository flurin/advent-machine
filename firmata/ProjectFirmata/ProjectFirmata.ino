...
#define FASTLED         0x0A // Pin mode
#define FASTLED_DATA   0x74 // 
#define FASTLED_SET    0x01 //  
...
void sysexCallback(byte command, byte argc, byte *argv)
{
  ...
  case FASTLED_DATA:
    ...
    if(subCommand == FASTLED_SET){
      ledNum = argv[1] + (argv[2] << 7); 
      leds[ledNum].r = (argv[3] + (argv[4] << 7)); 
      leds[ledNum].g = (argv[5] + (argv[6] << 7)); 
      leds[ledNum].b = (argv[7] + (argv[8] << 7));
    } else if (subCommand == FASTLED_SHOW){
      FastLED.show();
    }
