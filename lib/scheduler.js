...
module.exports = function(config){
  ...
  var schedule = function(){
    debug("Schedule!");
    // Get messages
    return Promise.all([config.messages.getPastDueDate(), config.messages.getImmideate()]).catch(function(err){
      console.log("Could not get messages", err);
    }).then(function(results){
      var messages = results[0];
      var immideateMessages = results[1];

      // Will just emit event.
      var done = Promise.resolve();
      if(immideateMessages && immideateMessages.length > 0){
        immideateMessages.forEach(function(message){
          done = done.then(function(){
            return config.queue.done(message);
          }).then(function(message){
            events.emit("immideate", message)
          })
        });
      };
