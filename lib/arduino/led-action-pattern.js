...
Pattern.prototype.action = function(){
  ...
  var perform = function(){
    logger.debug("LedActionPattern: PERFORM", self.name, self.id, self.run, self.count);
    ...
    patternSequence.then(function(){
      if(self.run){
        self.led.reset(); // Always blackout after end of pattern.
      ...
      if(err && err.code === "stop_run"){
        logger.debug("LedActionPattern: STOPPED in pattern", self.name, self.id);
        // That's ok.
        return self.led.reset();
      } else {
...
Pattern.prototype.start = function(led){
  ...
  this.run = true;
  this.action();
  logger.debug("LedActionPattern: START", this.name, this.id, this.run);
};
...
Pattern.prototype.stop = function(){
  ...
  this.led.reset();
  logger.debug("LedActionPattern: STOP", this.name, this.id, this.run);
};
